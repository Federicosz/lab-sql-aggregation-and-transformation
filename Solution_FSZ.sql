...
select floor(avg(length)/60) as 'hours'	,round(avg(length)%60) as 'minutes', concat(floor(avg(length)/60),'h',round(avg(length)%60),'min'),sec_to_time(avg(length)*60) from film;
...
select *,case when dayofweek(rental_date) between 0 and 4 then 'workday' else 'weekend' end as 'day_type' from rental;
/*select distinct day_type from rental; we don't know yet how to save it. however you can create other query to check it as select distinct dayname(rental_date),
case when dayofweek(rental_date) between 0 and 4 then 'workday' else 'weekend' end as 'day_type' from rental*/ 
